 ...
 * @property {boolean} [preserveVariables=false] - When used with `skipMapping=true`, this option will preserve the variables defined inside the container query in the output by injecting them to the root selector.
			...
			if (rule) {
				rule.walkDecls((decl) => {
					...
					for (let [prop, mappedValues] of conversionMap.entries()) {
						...
						// Create a random numerical only string of 7 characters to use as a postfix
						const randomPostfix = Math.random().toString().slice(2, 9);
						const replacement = checkForReplacement(decl, prop, [
							`--${identifierName}-${cleanPropertyName(prop, { selectorPrefix })}-${randomPostfix}`,
						], systemValues);
						...
						decl.value = decl.value.replace(new RegExp(decl.prop, "g"), `--${identifierName}-${cleanPropertyName(decl.prop, { selectorPrefix })}-${randomPostfix}`);
					}
				});
			}
